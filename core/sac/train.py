...
class SACTrainer:
    ...
    :param alpha: Alpha temperature coefficient used for promoting entropy in SAC. Only used as
        an initial value if automatic adjustment is enabled.
    :param alpha_min: Lower limit for the alpha temperature coefficient to use if specified. 
        Ignored if it equals to zero. 
    :param entropy_target: Entropy target value to use when the alpha temperature coefficient is
        dynamically adjusted and not fixed.
    :param auto_alpha: Boolean flag to enable automatic parameter adjustment of the alpha 
    ...
    def __init__(self, env_fn, wrappers_kwargs=dict(), use_gpu=False, model_path='', 
                 ...
                 clip_grad=False, alpha=0.2, alpha_min=0, entropy_target=None, 
                 auto_alpha=False, batch_size=100, start_steps=2500, learning_starts=1000, 
    ...
    def _calc_q_loss(self, alpha, obs: torch.Tensor, act: torch.Tensor, rew: torch.Tensor, 
        ...
        act_target, log_prob_target, _ = self.ac_mod.actor.log_prob_no_grad(obs_next)
    ...
    def _calc_pi_loss(self, alpha, obs: torch.Tensor, loss_mask: torch.Tensor):
        ...
        return pi_loss + self.pre_act_coeff * pre_act_loss, log_prob
