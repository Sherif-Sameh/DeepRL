...
class TD3Trainer:
    ...
    def __update_params(self, device, ac: MLPActorCritic, buf: ReplayBuffer, ac_optim: Adam, 
                        writer: SummaryWriter, epoch, gamma, polyak, target_noise, 
                        noise_clip, action_max, update_policy=False):
            ...
            for step in range(local_steps_per_epoch):
                ...
                if (buf.get_buffer_size() >= learning_starts) \
                    ...
                    for j in range(update_every):
                        ...
                        self.__update_params(device, ac_mod, buf, ac_optim, writer, epoch, 
                                             gamma, polyak, target_noise, noise_clip, 
